...
const matchesSlice = createSlice({
	...
	reducers: {
		...
		kickoffTimeUpdate(state, action) {
			// Find the index of the match(in case we want to update the kickoff time of existing match)
			const index = state.matches.findIndex((match) => match._id === action.payload._id);
			if (index !== -1) {
				state.matches[index] = {...state.matches[index], kickoffTime: action.payload.kickoffTime};
			}
		}
	},
});
